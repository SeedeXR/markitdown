...
/// Like `eprintln!` but never panics on a closed stderr pipe (e.g.
/// `markitdown … 2>&1 | head`). `eprintln!` aborts the thread on write error —
/// fatal inside a rayon batch worker — so all stderr logging goes through this.
macro_rules! elog {
...
fn stderr_progress() -> ProgressCallback {
    ...
    ProgressCallback::new(move |p: Progress| {
        if let Some(pct) = p.percent() {
            ...
            elog!("  [{:<7} {pct:>3}%] {}", p.phase, p.message);
        } else {
            last_pct.store(-1, Ordering::Relaxed);
            elog!("  [{:<12}] {}", p.phase, p.message);
        }
    })
}
...
fn convert_batch(md: &MarkItDown, args: &Cli) -> ExitCode {
    let Some(out_dir) = &args.output_dir else {
        elog!("error: multiple inputs require -O/--output-dir");
        return ExitCode::FAILURE;
    };
    if let Err(e) = std::fs::create_dir_all(out_dir) {
        elog!("error: creating {}: {e}", out_dir.display());
        return ExitCode::FAILURE;
    }
    ...
    if failures > 0 {
        elog!("{failures}/{} conversions failed", args.inputs.len());
        ExitCode::FAILURE
    } else {
        ExitCode::SUCCESS
    }
...
fn output_names(inputs: &[String]) -> Vec<PathBuf> {
    ...
    // First pass: disambiguate stem collisions by keeping the source extension
    ...
    let candidates: Vec<String> = inputs
        ...
        .map(|(input, name)| {
            if plain.iter().filter(|n| *n == name).count() > 1 {
                format!("{}.md", last_segment(input))
            } else {
                name.to_string_lossy().into_owned()
            }
        })
        .collect();
    // Second pass: guarantee global uniqueness so a batch can never silently
    // overwrite one output with another (e.g. `a/test.docx` + `b/test.docx`,
    // which collide even after the extension pass). Append `-N` before `.md`.
    ...
    match output {
        Some(path) => {
            if let Err(e) = std::fs::write(path, result.markdown.as_bytes()) {
                elog!("error: writing {}: {e}", path.display());
                return ExitCode::FAILURE;
            }
            ExitCode::SUCCESS
        }
...
fn print_or_pipe_ok(bytes: &[u8]) -> ExitCode {
    ...
    match stdout.write_all(bytes).and_then(|_| stdout.flush()) {
        ...
        Err(e) => {
            elog!("error: writing to stdout: {e}");
            ExitCode::FAILURE
        }
    }
}
